...
def _opai_relay_headers() -> Dict[str, str]:
    """Browser-like headers for the user's opai relay Cloudflare/WAF."""
    return {
        "User-Agent": (
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                extra["default_headers"] = _opai_relay_headers()
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    elif base_url_host_matches(sync_base_url, "opai.mangfu.fun"):
        async_kwargs["default_headers"] = _opai_relay_headers()
...
def resolve_provider_client(
    ...
    if provider == "custom":
        if explicit_base_url:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                extra["default_headers"] = {"User-Agent": "claude-code/0.1.0"}
            elif base_url_host_matches(custom_base, "opai.mangfu.fun"):
                extra["default_headers"] = _opai_relay_headers()
    ...
    try:
        ...
        if custom_entry:
            ...
            if custom_base:
                ...
                if base_url_host_matches(openai_base, "opai.mangfu.fun"):
                    _extra2["default_headers"] = _opai_relay_headers()
                logger.debug(
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            headers.update(_opai_relay_headers())
        elif base_url_host_matches(base_url, "api.githubcopilot.com"):
            from hermes_cli.copilot_auth import copilot_request_headers
    ...
    if pconfig.auth_type == "external_process":
        ...
        final_model = _normalize_resolved_model(
            model
            or _API_KEY_PROVIDER_AUX_MODELS.get(provider, "")
            or (main_runtime.get("model") if main_runtime else None)
            or _read_main_model()
            or _API_KEY_PROVIDER_AUX_MODELS.get("copilot-acp", "")
            or _API_KEY_PROVIDER_AUX_MODELS.get("copilot", "")
            or "gpt-5-mini",
            provider,
        )

...
Storage (``~/.hermes/auth/google_antigravity_oauth.json``, chmod 0o600):
...
_DEFAULT_CLIENT_ID = (
    '1071006060591-tmhssi'
    'n2h21lcre235vtolojh4'
    'g403ep.apps.googleus'
    'ercontent.com'
)
...
class GoogleOAuthError(RuntimeError):
    ...
    def __init__(self, message: str, *, code: str = "google_antigravity_oauth_error") -> None:
...
def _credentials_path() -> Path:
    return get_hermes_home() / "auth" / "google_antigravity_oauth.json"
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise TimeoutError(
                            f"Timed out acquiring Google Antigravity OAuth credentials lock at {lock_file_path}."
                        )
                    time.sleep(0.05)
        else:
            try:
...
def _require_client_id() -> str:
    ...
    if not cid:
        raise GoogleOAuthError(
            ...
            code="google_antigravity_oauth_client_id_missing",
        )
    return cid
...
def _post_form(url: str, data: Dict[str, str], timeout: float) -> Dict[str, Any]:
    ...
    try:
        ...
        code = "google_antigravity_oauth_token_http_error"
        if "invalid_grant" in detail.lower():
            code = "google_antigravity_oauth_invalid_grant"
        raise GoogleOAuthError(
        ...
        raise GoogleOAuthError(
            ...
            code="google_antigravity_oauth_token_network_error",
        ) from exc
    ...
    *,
    ...
    if not refresh_token:
        raise GoogleOAuthError(
            ...
            code="google_antigravity_oauth_refresh_token_missing",
        )
    cid = client_id if client_id is not None else _get_client_id()
    csecret = client_secret if client_secret is not None else _get_client_secret()
    data = {
...
def get_valid_access_token(*, force_refresh: bool = False) -> str:
    ...
    credential file is wiped and a ``google_antigravity_oauth_invalid_grant`` error is raised
    (caller is expected to trigger a re-login flow).
    """
    creds = load_credentials()
    if creds is None:
        raise GoogleOAuthError(
            "No Google Antigravity OAuth credentials found. Run `hermes login --provider google-antigravity` first.",
            code="google_antigravity_oauth_not_logged_in",
        )
    ...
    try:
        try:
            ...
            if exc.code == "google_antigravity_oauth_invalid_grant":
                logger.warning(
        ...
        if not new_access:
            raise GoogleOAuthError(
                ...
                code="google_antigravity_oauth_refresh_empty",
            )
    ...
    *,
        ...
        if ready.wait(timeout=callback_wait_seconds):
            ...
            if error:
                raise GoogleOAuthError(
                    ...
                    code="google_antigravity_oauth_authorization_failed",
                )
        else:
    ...
    if not code:
        raise GoogleOAuthError(
            ...
            code="google_antigravity_oauth_no_code",
        )
...
def _paste_mode_login(
    ...
    code = _prompt_paste_fallback()
    if not code:
        raise GoogleOAuthError("No authorization code provided.", code="google_antigravity_oauth_no_code")
    ...
    *,
    ...
    if not access_token or not refresh_token:
        raise GoogleOAuthError(
            ...
            code="google_antigravity_oauth_incomplete_token_response",
        )

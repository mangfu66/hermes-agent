...
def _add_column_if_missing(
    ...
    (issue #21708). ``connect()`` / ``init_db()`` can race across threads
    or processes on the same on-disk board during cold start; both
    initializers may read the same pre-migration PRAGMA snapshot, then
    one wins the column-add while the other hits ``duplicate column
    name`` a moment later. That second case is not a real schema failure.
    """
    try:
...
def _terminate_reclaimed_worker(
    ...
    *,
    ...
    # Already dead / reaped workers do not need a best-effort signal.
    # This matters both in production stale-reclaim paths and in tests that
    # intentionally use fake PIDs to exercise reclaim logic.

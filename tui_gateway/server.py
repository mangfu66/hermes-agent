...
def _resolve_runtime(requested: str | None = None, target_model: str | None = None) -> dict:
    """Resolve the effective runtime so TUI matches classic CLI routing."""
    model = (target_model or _resolve_model() or "").strip() or _resolve_model()
    runtime = {
        "model": model,
        "provider": "",
        "base_url": "",
    ...
    try:
        ...
        if target_model:
            kwargs["target_model"] = model or None
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    runtime = _resolve_runtime(requested_provider, model or None) if explicit_override else _resolve_runtime()
    return AIAgent(
        model=runtime.get("model") or model or _resolve_model(),

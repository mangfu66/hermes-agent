...
def _(rid, params: dict) -> dict:
    try:
        ...
        agent = session.get("agent") if session else None
        cfg = _load_cfg()
        runtime_model, runtime_provider = _resolve_startup_runtime()
        current_provider = getattr(agent, "provider", "") or runtime_provider or ""
        current_model = getattr(agent, "model", "") or runtime_model or _resolve_model()
        current_base_url = getattr(agent, "base_url", "") or ""

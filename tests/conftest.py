...
def _live_system_guard(request, monkeypatch):
    ...
    def _guarded_kill(pid, sig, *args, **kwargs):
        # POSIX ``kill(pid, 0)`` is a non-delivering existence probe used by
        # liveness helpers such as kanban_db._pid_alive(). Allow it even for
        # foreign PIDs so tests can safely check process existence without
        # tripping the live-system guard.
